...
public class Computer_c extends Board {
    // 記号用の変数を保存するインスタンス変数
    private char symbol;
    ...
    public boolean put() {
        ...
        if(check()) {
            // 置くことができる場合はランダムな場所に記号を設置
            // ランダムな数値は、getRandNum()メソッドを通じて、重複なく取得
            int rowIndex = getRandNum() / 3;
            int colIndex = getRandNum() % 3;
